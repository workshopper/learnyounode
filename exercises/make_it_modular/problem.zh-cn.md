...
编写一个程序来打印出所给文件目录的所含文件列表，并且以特定的文件名后缀来过滤这个列表。将会提供两个参数，第一个参数是要列举的目录，第二个参数是要过滤的文件爱你扩展名。你需要在终端中打印出过滤出来的文件列表（一个文件一行）。此外，你**必须**使用异步 I/O。

你得编写一个*模块* 文件去做大部分的事情。这个模块必须*导出（export）*一个函数，这个函数将接收**三个**参数：目录名、文件扩展名、回调函数，并按此顺序传递。文件扩展名必须和传递给你的程序的扩展名字符串一模一样。也就是说，请不要把它转成正则表达式或者加上 "." 前缀或者做其他的处理，而是直接传到你的模块中去，在那里，你可以做一些处理来使你的过滤器能正常工作。
...
通过创建一个仅包含目录读取和文件过滤相关的函数的文件来建立一个新的模块。要定义*单一函数*（*single function*）的*导出*（*export*），你可以将你的函数赋值给 `module.exports` 对象：
...
现在，`mymodule` 这个变量就指向了你的模块中  `module.exports`了，因为你刚导出了一个单一的函数，所以现在 `mymodule` 就是那个函数了，你可以直接调用它！
